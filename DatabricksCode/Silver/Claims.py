...
claims_df.select(['datasource']).filter(claims_df.datasource != 'hosb').show(5,False)
...
# MAGIC %sql
# MAGIC CREATE TABLE IF NOT EXISTS tt_hc_adb_ws.silver.claims (
...
# MAGIC MERGE INTO tt_hc_adb_ws.silver.claims AS target
...
# MAGIC %sql
# MAGIC select * from  tt_hc_adb_ws.silver.claims
